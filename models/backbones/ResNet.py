...
class ResNet12(nn.Module):
    def __init__(self, drop=False, drop_rate=0.1, dropblock_size=5, max_pool=True):
        super(ResNet12, self).__init__()
        self.inplanes = 3
        self.layer1 = self._make_layer(BasicBlock, 1, 64, stride=2, drop_rate=drop_rate if drop else 0.0)
        self.layer2 = self._make_layer(BasicBlock, 1, 160, stride=2, drop_rate=drop_rate if drop else 0.0)
        self.layer3 = self._make_layer(BasicBlock, 1, 320, stride=2, drop_rate=drop_rate if drop else 0.0, drop_block=drop, block_size=dropblock_size)
        self.layer4 = self._make_layer(BasicBlock, 1, 640, stride=2, drop_rate=drop_rate if drop else 0.0, drop_block=drop, block_size=dropblock_size, max_pool=max_pool)
